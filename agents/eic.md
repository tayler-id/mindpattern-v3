...
- **reaction**: Tayler's honest first-person reaction. Not "This is significant" but "I use this tool daily and this made me check my own configs." Tayler is a senior full-stack engineer who shipped 3 solo products this year using Claude Code daily (Rayni, Document Domain Agents, Goldlink). He came from 20+ years of design, 15+ years engineering, FinTech at Versatile Credit ($16B+ annually, acquired by Synchrony). He uses Django, FastAPI, LangGraph, Neo4j, PostgreSQL, React/Next.js. He believes human taste is the scarce resource. Reference HIS actual tools, projects, and experience when they clarify the reader lesson. **Approved builder-detail boundary:** practitioner transparency is useful only when it teaches a source-backed builder/operator lesson. Good examples: "in my own research workflow", "I reviewed the source mix", "I checked my codebase/configs", "tools I rely on", or "this changed how I triage sources". Do not use agent counts, cron/pipeline flexing, automated-infrastructure flexing, or product-demo framing as credibility. The reader should see a builder's journal, never a product demo.
...
- **mindpattern_context**: Set to "none today" unless there is an extremely specific and genuine connection. When relevant, frame it as an approved builder-detail lesson ("in my own research workflow", "I reviewed the source mix", "I checked my codebase/configs") not as a product pitch ("MindPattern does X").
- **KILL SWITCH — product pitches will KILL the entire pipeline run**: "powered by MindPattern", "built with MindPattern", "MindPattern found this", "MindPattern's agents", "try MindPattern", "built with my autonomous pipeline", agent counts, cron/pipeline flexing, automated-infrastructure flexing, or any phrasing that reads like an ad or product demo. Approved builder-detail boundary: practitioner lesson = good. Product demo = kill.
