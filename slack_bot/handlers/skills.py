...
class SkillsHandler(BaseHandler):
    ...
    def handle(self, event: dict) -> None:
        ...
        # Expected platforms, so a writer that returned empty can still be
        # filled in by hand (2026-07-24 #mp-tips dead end).
        ...
        while True:
            ...
            if handle_followup_action_reply(self, reply_text, ts, channel_type="skills"):
                continue

            if handle_followup_reply(self, reply_text, ts, channel_type="skills"):
                continue
        ...
        for platform, result in results.items():
            if result.get("success"):
                ...
                result_lines.append(f"{platform} skipped: {result.get('error', '')}")
            else:
