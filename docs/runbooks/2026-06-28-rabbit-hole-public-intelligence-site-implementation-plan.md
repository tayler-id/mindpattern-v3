...
| 16 | 4 | Upgrade Wire to story/feed rows | Partial | Wire now prefers v3 `/api/feed` and falls back to older findings API until backend production deploy happens. Story-feed artifacts are pending, so Wire rows still point to finding pages. Briefing story units with source refs now link to `/s/<story-id>`. Local smoke: `/` -> 200 and renders real feed counts/items; `/briefings/2026-06-28` -> 200 and includes `/s/2026-06-28-agents`. |
...
| 21 | 6 | Add browser smoke checks | Partial | Local HTTP smoke completed because no browser MCP is exposed in this session: `/`, `/f/13776`, `/briefings/2026-06-10`, `/briefings/2026-06-28`, `/source/techcrunch.com`, `/e/openai`, `/s/2026-06-28-agents` -> 200; v3 `/api/feed`, `/api/related/13776`, `/api/issues/2026-06-10/structured`, `/api/entities/openai`, `/api/stories`, and `/api/stories/2026-06-28-agents` -> 200; `POST /api/chat` -> 410. Full visual/mobile browser smoke remains pending before production release. |
