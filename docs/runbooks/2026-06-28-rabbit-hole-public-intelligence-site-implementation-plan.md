...
| 5 | 2 | Add public story artifact contracts | Partial | Added safe `story_artifact_path()` and `build_public_story()` contracts for source-backed public story output, provenance, source/entity refs, related paths, JSON-LD-ready metadata, and no-source suppression. Persisted `reports/<user>/site-stories/` artifact generation/backfill remains pending. Verification: `.venv/bin/python3 -m pytest tests/test_site_issue_contracts.py -q` -> 8 passed. |
...
| 5B | 2 | Add newsletter splitter and entity linker | Partial | Deterministic markdown splitter extracts sections, H2-only issue story units, source refs, and public entity refs. Added stopword/shape filtering to remove sentence-noise entities while preserving company/tool names. Finding/arc linking is still unresolved rather than fabricated. Verification: `tests/test_site_issue_contracts.py` -> 6 passed. |
...
| 6 | 2 | Add confidence gate and safety tests | Partial | Public story builder only emits stories with source refs and marks them `source-backed`; unsafe users/slugs and traversal attempts return safe 404s. Broader site-content confidence scoring and degraded/draft artifact states remain pending. Verification: `.venv/bin/python3 -m pytest tests/test_api_contract.py tests/test_auth_middleware.py tests/test_site_issue_contracts.py -q` -> 45 passed, 1 known Starlette/httpx warning. |
...
| 11 | 3 | Expose public story APIs | Yes | Added public read-only `GET /api/stories` and `GET /api/stories/{slug}` over canonical newsletter structured issues. Endpoints validate user/slug/date input, return only source-backed public stories, and use safe 404s for missing/unsafe input. Verification: `.venv/bin/python3 -m pytest tests/test_api_contract.py::test_story_endpoints_return_source_backed_public_stories -q` -> 1 passed. Local smoke: v3 `/api/stories?user=ramsay&limit=3` -> 200; `/api/stories/2026-06-28-agents?user=ramsay` -> 200. |
| 12 | 3 | Add story API contract fixtures | Yes | Added contract fixtures covering story list/detail, source refs, provenance, confidence, traversal-safe 404s, unsafe users, and auth public allowlist for `/api/stories`. Verification: `.venv/bin/python3 -m pytest tests/test_api_contract.py tests/test_auth_middleware.py tests/test_site_issue_contracts.py -q` -> 45 passed, 1 known Starlette/httpx warning. |
| 12A | 3 | Expose structured issue/entity/source APIs | Partial | Added public `/api/issues`, `/api/issues/{date}/structured`, and `/api/entities/{slug}` from canonical newsletter reports. Entity API validates slugs/users and returns only source-backed story excerpts/source refs/provenance. Source detail API remains pending; Rabbit Hole still derives source pages from existing `/api/sources` + `/api/findings`. Verification: `.venv/bin/python3 -m pytest tests/test_api_contract.py tests/test_auth_middleware.py tests/test_site_issue_contracts.py -q` -> 42 passed, 1 known Starlette/httpx warning. |
| 12B | 3 | Add structured public graph API fixtures | No | Not started. |
| 13 | 4 | Add Rabbit Hole story types/helpers | Yes | Added Rabbit Hole API types/helpers for feed, related findings, structured issues, source trails, single-finding fallback compatibility, `PublicStory`, `StoriesResponse`, `getStories()`, and `getStory()`. Verification: Rabbit Hole `pnpm lint`, `pnpm exec tsc --noEmit --incremental false`, and `pnpm build` passed. |
| 14 | 4 | Build `/s/[slug]` story page | Yes | Added dynamic `/s/[slug]` public story page with safe slug validation, Article JSON-LD, source trail, entity links, related paths, confidence labels, and provenance. Local smoke with v3 on `127.0.0.1:8010` and Rabbit Hole on `127.0.0.1:3010`: `/s/2026-06-28-agents` -> 200 and rendered `Public story`, `Source trail`, `Provenance`, and source text. |
| 14A | 4 | Build dynamic `/e/[slug]` and `/source/[domain]` pages | Yes | Implemented `/source/[domain]` with safe domain validation, source JSON-LD, source stats, and related findings. Implemented `/e/[slug]` with safe slug validation, entity JSON-LD, story trail, issue links, and source links over `/api/entities/{slug}`. Local smoke: `/source/techcrunch.com` -> 200; `/e/openai` -> 200. |
...
| 16 | 4 | Upgrade Wire to story/feed rows | Yes | Wire normal views now prefer v3 `/api/stories` and render source-backed `StoryWireRow` items that link to `/s/<slug>`, with source count, entity count, confidence, and first-source evidence. If stories are unavailable, the existing feed/findings fallback still renders. Topics view remains finding-grouped for now. Local smoke: `/` -> 200 and includes `/s/2026-06-28-agents` plus `source-backed`; `/?view=latest` -> 200 and includes `/s/2026-06-28-agents`; `/?view=topics` -> 200 and includes `/f/` fallback rows. |
...
| 17 | 4 | Add briefing-to-story modules | Partial | Briefing `Thread summary` now links source-backed story units to dynamic `/s/<slug>` pages while preserving the full canonical newsletter below. A dedicated richer story-link module with arc/media cross-links remains pending. Local smoke: `/briefings/2026-06-28` -> 200 and rendered `Thread summary` plus `/s/2026-06-28-agents`. |
...
| 21 | 6 | Add browser smoke checks | Partial | Local HTTP smoke completed because no browser MCP is exposed in this session: `/`, `/?view=latest`, `/?view=topics`, `/f/13776`, `/briefings/2026-06-10`, `/briefings/2026-06-28`, `/source/techcrunch.com`, `/e/openai`, `/s/2026-06-28-agents` -> 200; homepage/latest render `/s/2026-06-28-agents` and `source-backed`; topics renders `/f/` fallback rows. v3 `/api/feed`, `/api/related/13776`, `/api/issues/2026-06-10/structured`, `/api/entities/openai`, `/api/stories`, and `/api/stories/2026-06-28-agents` -> 200; `POST /api/chat` -> 410. Full visual/mobile browser smoke remains pending before production release. |
| 22 | 6 | Run Graphify, docs, handoff | Yes | Updated Done table and handoff with committed branches, local smoke, verification, and remaining blockers. `git diff --check` passed. Latest `graphify update .` rebuilt 7,403 nodes, 11,906 edges, 463 communities; HTML viz skipped due 5,000-node default. `graphify check-update .` passed. |
