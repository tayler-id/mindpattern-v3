...
| 24 | 4 | Add historical structured issue backfill audit | No | 2026-07-01 archive hardening completed: `/api/reports` now filters placeholder/prompt-error/debug reports, prefers canonical `YYYY-MM-DD.md` files over suffix variants, strips assistant preamble from metadata/detail content, and exposes `word_count`/`content_status` without fabricating graph links. Full historical structured backfill remains pending: normalize old newsletters into site-issues with parsed/skipped/unresolved/redacted counts and no fabricated links. |
