...
|---|---|---|---|---|
| 0 | 0 | Capture current dirty state in both repos | Yes | 2026-06-30: v3 on `feature/rabbit-hole-public-intelligence-site` has pre-existing dirty `data/ramsay/**`, `data/social-drafts/eic-topic.json`, `run-launchd.sh`, `tests/test_launchd_wrapper.py`, plus new spec/handoff docs. Rabbit Hole on `feature/rabbit-hole-public-intelligence-site` has `M src/components/wire/wire-row.tsx` and untracked `docs/specs/2026-06-26-backend-{spec,plan-tasks}.md`. |
...
| 2 | 0 | Add implementation plan for the real content machine | Yes | 2026-06-30: this plan added. Remaining verification: owner review before implementation goal. |
| 3 | 1 | Add public artifact contract tests | No | Define tests for safe paths, statuses, redaction, publishability, and date/user/slug validation. |
| 4 | 1 | Implement artifact contracts and writers | No | Add typed helpers for site run ledger, corpus inventory, candidates, graph packs, stories, dossiers, and collections. |
| 5 | 1 | Make `/api/stories` accept dated site-story directories | No | Current API reads `reports/<user>/site-stories/**/*.json`; tests must pin dated path behavior and reject draft/degraded/parser output. |
| 6 | 1 | Add site run ledger and corpus inventory APIs | No | `GET /api/site/runs/{date}` and `/api/site/corpus/{date}` with safe path validation. |
| 7 | 2 | Inventory actual corpus graph sources | No | Record existing tables/artifacts, public-safe fields, counts, and gaps without committing private data. |
| 8 | 2 | Add corpus graph read model | No | Centralize reads from findings, embeddings, kg tables, entity_graph, sources, issues, arcs, and followups. |
| 9 | 2 | Add paginated entity index API | No | `/api/entities?q=&limit=&offset=` over full corpus with totals/has_more where safe. |
| 10 | 2 | Harden entity detail and neighbors APIs | No | `/api/entities/{slug}` and `/neighbors` include counts, evidence, pagination, relationships, and public-safe totals. |
| 11 | 2 | Add source detail API | No | `/api/sources/{domain}` validates domains and returns source history, linked findings, entities, arcs, and issue mentions. |
| 12 | 2 | Add public finding and arc detail APIs | No | `/api/findings/{id}` or compatible route plus `/api/narrative-arcs/{id}` expose evidence and graph links safely. |
| 13 | 2 | Add multi-connector related path scorer | No | Score same entity/source/semantic/arc/update/contrast/role/threat/stack/policy/followup/newsletter connectors with reader labels. |
| 14 | 3 | Add graph pack fixtures | No | Public-safe fixtures cover finding story, arc update, entity dossier, source watch, contrast pair, weak input, and follow-up update. |
| 15 | 3 | Implement graph pack builder | No | Builds bounded evidence packs from corpus read model and fixtures; weak data degrades visibly. |
| 16 | 3 | Implement deterministic candidate selector | No | Ranks candidates by importance, source strength, recency, entity recurrence, arcs, semantic density, novelty, and duplication risk. |
| 17 | 3 | Add expert provider interface and fake experts | No | Assignment Editor, Graph Cartographer, Domain Expert, Skeptic, Narrative Editor, Source Librarian, GEO Editor, Website Publisher. |
| 18 | 3 | Implement deterministic story generator | No | Fixture graph pack becomes readable site-native story artifact with title/dek/take/why-now/body/source trail/graph trail. |
| 19 | 3 | Implement confidence gate | No | Published requires high confidence, source evidence, claim evidence, graph evidence, redaction, why-now, no bad headings/raw markdown. |
| 20 | 3 | Implement artifact writer and dry-run service | No | Writes run ledger, corpus inventory, candidates, graph packs, published/draft/degraded stories under `reports/<user>/site-*`. |
| 21 | 3 | Add CLI or focused command for content-machine dry run | No | Runs one date/user in dry-run mode without full daily pipeline, network, Slack, email, social APIs, Fly, Vercel, or live providers. |
...
| 26 | 5 | Split Rabbit Hole frontend types hard | No | `IssueStoryUnit`, `PublicSiteStory`, `GraphPack`, `EntityPage`, `SourcePage`, `FindingPage`, and `ArcPage` cannot be confused. |
| 27 | 5 | Rebuild homepage against content-machine contract | No | Published site stories first, graph/feed fallback second, no parser chunks, no broken chat-first primary UX. |
| 28 | 5 | Rebuild public story page | No | `/s/<slug>` renders site-native artifact with readable prose, source trail, graph trail, related paths, provenance, JSON-LD. |
...
| 30 | 5 | Rebuild entity/source/finding/arc pages | No | Dynamic templates show counts, pagination, source evidence, public story links, and readable relationships. |
| 31 | 5 | Disable or ground chat primary entry | No | Chat must not be primary unless grounded in public graph APIs; otherwise hide/disable from first-screen flow. |
| 32 | 6 | Run backend focused and safety tests | No | Run content, graph, API, auth, runner dry-run suites. |
| 33 | 6 | Run Rabbit Hole lint/typecheck/build | No | `pnpm lint`, `pnpm exec tsc --noEmit --incremental false`, `pnpm build`. |
| 34 | 6 | Run local browser smoke | No | Homepage, story, briefing, entity, source, finding, arc, hidden chat, mobile/desktop, no raw markdown/bad headings/internal labels, links work first click. |
| 35 | 6 | Run Graphify and update handoff/docs | No | `graphify update .`, `graphify check-update .`, Done rows/evidence updated. |
