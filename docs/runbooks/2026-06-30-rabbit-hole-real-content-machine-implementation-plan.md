...
| 29 | 5 | Rebuild briefing enrichment links | No | 2026-07-01 corrective action: removed the parser-derived `IssueThreadSummary` from canonical briefing pages because it displayed line-clamped story snippets above the full report and made newsletters look truncated. Follow-up archive fix: `/briefings` is now dynamic, archive previews are no longer CSS-clamped, and backend report cleaning hides prompt-error/placeholder rows plus assistant preambles. Full newsletters render directly from canonical cleaned markdown. Proper graph enrichment remains pending and must link to entity/source/finding/arc and only to `/s` when a published site-story artifact exists. |
