...
| 22 | 4 | Add post-newsletter runner hook | Yes | 2026-07-01: added skippable `site_content` phase after `deliver` and before `learn`. It calls deterministic corpus content generation only, writes ignored `reports/<user>/site-*` artifacts, can be disabled with `MP_SITE_CONTENT_DISABLED=1`, and returns failed/degraded status without throwing so newsletter delivery remains autonomous. Verification: focused runner tests passed. |
| 23 | 4 | Add trace/logging integration | Yes | 2026-07-01: `site_content_started`, `site_content_completed`, and `site_content_failed` events now log compact public-safe counts/status only; artifact paths and raw story content are not saved to trace payloads. Verification: `tests/test_runner.py::TestSiteContentPhase` checks compact trace payloads and fail-open behavior. |
| 24 | 4 | Add historical structured issue backfill audit | Yes | 2026-07-01: added deterministic `run_site_issue_backfill` plus `tools/site-issue-backfill.py`. It writes `site-issues/<date>.json` and `site-issue-backfills/<run-date>.json`, records parsed/skipped/unresolved/redacted/degraded counts, prefers canonical reports, skips placeholders/dry-runs, and leaves missing finding/entity/source matches unresolved instead of inventing IDs. Verification: `.venv/bin/python3 -m pytest tests/test_site_issue_backfill.py -q` -> 3 passed; CLI smoke in `/private/tmp/mp-site-issue-backfill-smoke` parsed 1 report. |
| 25 | 4 | Add high-confidence historical seed generation | Yes | 2026-07-01: added `run_historical_site_seed_generation`; it requires the backfill audit, builds internal graph packs from audited structured issues, publishes only high-confidence source-backed historical `site_story` artifacts, and records published/degraded/skipped decisions in `site-historical-seeds/<run-date>.json`. Verification: `.venv/bin/python3 -m pytest tests/test_site_issue_backfill.py tests/test_site_content_contracts.py tests/test_site_content_engine.py tests/test_site_content_confidence.py tests/test_site_content_api.py -q` -> 24 passed, 1 warning; CLI smoke with `--seed` published 1 temp story. |
...
| 27 | 5 | Rebuild homepage against content-machine contract | Yes | 2026-07-01: Rabbit Hole homepage now renders `/api/stories` published site artifacts first and keeps `/api/feed`/findings underneath so one generated story cannot collapse the whole Wire. Local smoke with fixture story rendered `/s/openai-agent-runtime-reliability` plus `/f/...` rows on `/`, with no parser chunk headings. |
...
| 29 | 5 | Rebuild briefing enrichment links | Yes | 2026-07-01 corrective action: removed the parser-derived `IssueThreadSummary` from canonical briefing pages because it displayed line-clamped story snippets above the full report and made newsletters look truncated. Follow-up archive fix: `/briefings` is now dynamic, archive previews are no longer CSS-clamped, and backend report cleaning hides prompt-error/placeholder rows plus assistant preambles. Full newsletters render directly from canonical cleaned markdown. Final graph-enrichment slice: `/api/issues/{date}/structured` now includes `published_story_refs` and per-story `published_story` only when a real published story artifact maps to the story unit; Rabbit Hole `/briefings/[date]` renders a post-newsletter Graph trail with source, entity, and story paths, using no fake `/s` links and no parser/internal connector labels. Story detail truncation fix: `/api/stories/{slug}` now falls back to a full source-backed structured newsletter story when no AI-written `site-stories` artifact exists, while AI artifacts still win; Rabbit Hole `/s/<slug>` no longer shows the 280-character summary as the article body. Follow-up quality fix: `/api/stories` now reports full `total` and `has_more` metadata instead of partial loaded-page counts, and briefing graph entity display filters noisy filler entities such as `that`, `what`, `they`, and `worth`. Verification: focused backend contract tests -> 3 passed; Rabbit Hole `pnpm lint`, `pnpm exec tsc --noEmit --incremental false`, and `pnpm build` passed; local smoke `/briefings/2026-06-29` rendered Graph trail/source/entity links and no `SHARED SOURCE`, `shared_source`, `shared_entity`, `Loading related signals`, `line-clamp-2`, `Now let me write`, or `Prompt is too long`; dynamic story smoke returned full `body_markdown` length greater than its preview summary; `/api/stories?limit=1` returned `total=3976` and `has_more=true`; rendered briefing no longer linked `/e/that`, `/e/what`, `/e/they`, or `/e/worth`. |
...
| 32 | 6 | Run backend focused and safety tests | Yes | 2026-07-01: `.venv/bin/python3 -m pytest tests/test_site_content_contracts.py tests/test_site_content_engine.py tests/test_site_content_confidence.py tests/test_site_content_experts.py tests/test_site_content_api.py tests/test_site_graph_read_model.py tests/test_site_graph_api.py tests/test_site_related_paths.py tests/test_api_contract.py tests/test_auth_middleware.py -q` -> 69 passed, 1 warning. Reran after historical backfill/seed slice, briefing graph enrichment, dynamic story-body fallback, accurate stories pagination metadata, and entity-noise filtering: `.venv/bin/python3 -m pytest tests/test_site_issue_backfill.py tests/test_site_content_contracts.py tests/test_site_content_engine.py tests/test_site_content_confidence.py tests/test_site_content_experts.py tests/test_site_content_api.py tests/test_site_graph_read_model.py tests/test_site_graph_api.py tests/test_site_related_paths.py tests/test_api_contract.py tests/test_auth_middleware.py tests/test_runner.py::TestSiteContentPhase tests/test_runner.py::TestDryRunPhases -q` -> 82 passed, 1 warning. |
| 33 | 6 | Run Rabbit Hole lint/typecheck/build | Yes | 2026-07-01: Rabbit Hole `pnpm lint`, `pnpm exec tsc --noEmit --incremental false`, and `pnpm build` passed. Reran after briefing archive unclamp/dynamic fix, briefing Graph trail wiring, story-page summary/body fix, and stories `has_more` type/entity display filter; all three still passed and build marked `/briefings` and `/briefings/[date]` dynamic. |
| 34 | 6 | Run local browser smoke | Yes | 2026-07-01: local v3 `127.0.0.1:8010` + Rabbit Hole `127.0.0.1:3010`; backend `/api/stories` -> 200; frontend `/`, `/s/openai-agent-runtime-reliability`, `/briefings`, `/briefings/2026-06-17`, `/e/openai`, `/source/openai.com`, `/f/101`, `/arc/agent-runtime-control-plane?date=2026-07-01` -> 200. Homepage smoke after the Wire-population fix showed the generated story plus normal `/f/...` rows. Briefing smoke after removing `IssueThreadSummary` showed the full report ending (`How This Newsletter Learns From You` and reply footer) with no top `Thread summary` block. HTML scan found no `Top 5 Stories Today`, raw `**` markdown, `SHARED SOURCE`, `shared_source`, or `shared_entity` internal labels on the new content-machine pages. Archive/detail smoke after report cleanup: `/api/reports` and `/briefings` contain no `Prompt is too long`, `Now let me write`, `2026-02-19`, or `line-clamp-2`; `/briefings/2026-02-20` renders `Ramsay Research Agent - 2026-02-20` with `4,864` words. Graph-enriched briefing smoke: `/api/issues/2026-06-29/structured` -> 200 with `published_story_refs`; `/briefings/2026-06-29` -> 200 and rendered `Graph trail`, `Story paths`, `/source/theverge.com`, and `/e/openai`; scan found no `SHARED SOURCE`, `shared_source`, `shared_entity`, `Loading related signals`, `line-clamp-2`, `Now let me write`, or `Prompt is too long`. Story truncation smoke: `/api/stories/2026-06-29-china-s-z-ai-shipped-an-open-weight-model-that-matches-an-export-banned-one-on-security-work` returned source-backed full body markdown; Rabbit Hole `/s/<slug>` rendered the full Story section instead of the clipped summary header. Final metadata/noise smoke: `/api/stories?user=ramsay&limit=1` returned 200 with one item, `total=3976`, and `has_more=true`; `/`, `/briefings/2026-06-29` returned 200; briefing still rendered `How This Newsletter Learns From You`, `Quick feedback template`, and `Graph trail`, and no longer rendered `/e/that`, `/e/what`, `/e/they`, or `/e/worth`. |
| 35 | 6 | Run Graphify and update handoff/docs | Yes | 2026-07-01: `graphify update .` rebuilt 7,742 nodes, 12,587 edges, and 452 communities; HTML visualization skipped because the graph exceeds the 5,000-node default. `graphify check-update .` passed. Handoff and Done rows updated with evidence. Reran after historical backfill/seed slice: `graphify update .` rebuilt 7,794 nodes, 12,787 edges, and 463 communities; `graphify check-update .` passed. Reran after briefing enrichment: `graphify update .` rebuilt 7,796 nodes, 12,793 edges, and 468 communities; `graphify check-update .` passed. |
| 36 | 6 | Commit, push, PR/merge readiness | Yes | 2026-07-01: v3 and Rabbit Hole branches were pushed with phase-scoped commits. Follow-up archive cleanup commits were added for v3 (`fix: clean public briefing archive reports`) and Rabbit Hole (`fix: render live briefing archive previews`). Unrelated dirty files remain unstaged and are listed in final status/handoff. |
| 37 | 6 | Release gate | Ready - approval required | No Vercel production deploy was run. Exact remaining step: owner explicitly approves production Vercel deploy for the pushed Rabbit Hole `feature/rabbit-hole-public-intelligence-site` branch. Fly deploy is not needed unless explicitly approved. |
...
**Acceptance criteria:**
- [x] Backfill is resumable and idempotent.
- [x] Audit records parsed, skipped, unresolved, redacted, and degraded counts.
- [x] Backfill does not invent missing finding/entity/source matches.

**Verification:**
- [x] `.venv/bin/python3 -m pytest tests/test_site_issue_backfill.py -q`
...
**Acceptance criteria:**
- [x] Seed generation uses audited structured issues and graph packs.
- [x] Medium/degraded historical candidates remain internal.
- [x] Ledger records why each candidate was published, drafted, degraded, or
      skipped.

**Verification:**
- [x] `.venv/bin/python3 -m pytest tests/test_site_content_engine.py tests/test_site_issue_backfill.py -q`
...
- [x] Newsletter remains autonomous.
- [x] Site content run status is visible.
- [x] Historical content has an audit trail before broad publishing.
